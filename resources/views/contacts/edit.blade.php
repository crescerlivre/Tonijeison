            ...
            <div class="card row pb-3">
                <form  method="POST" action="{{ route('contacts.update', $contacts->id) }}" method="patch" class="row">
                    ...
                    <div class="mb-3 col-3">
                        <label for="date_birthday" class="form-label">Data aniversário</label>
                        <input value="{{ $contacts->date_birthday }}" type="tel" class="form-control" id="date_birthday" name="date_birthday" placeholder="00/00" required>

                        @if ($errors->has('date_birthday'))
                        <span class="text-danger text-left">{{ $errors->first('date_birthday') }}</span>
                        @endif
                    </div>
